basicEventSelectionDict = {
    "m_applyGRLCut" : True,
    "m_GRLxml" : "$ROOTCOREBIN/data/SUSYTools/GRL/Moriond2016/data15_13TeV.periodAllYear_DetStatus-v73-pro19-08_DQDefects-00-01-02_PHYS_StandardGRL_All_Good_25ns.xml",
    "m_useMetaData" : True,
    "m_cleanPowheg" : True,
    "m_doPUreweighting" : False,
    "m_lumiCalcFileNames" : "$ROOTCOREBIN/data/FactoryTools/ilumicalc_histograms_None_276262-304494_OflLumi-13TeV-005.root",
    "m_PRWFileNames" : "/cvmfs/atlas.cern.ch/repo/sw/database/GroupData/dev/SUSYTools/merged_prw_mc15c.root",
    "m_PU_default_channel" : 0,
    "m_vertexContainerName" : "PrimaryVertices",
    "m_applyPrimaryVertexCut" : True,
    "m_PVNTrack" : 2,
    "m_applyEventCleaningCut" : True,
    "m_applyCoreFlagsCut" : True,
    "m_triggerSelection" : "",
    "m_applyTriggerCut" : False,
    "m_storeTrigDecisions" : True,
    "m_storePassL1" : False,
    "m_storePassHLT" : True,
    "m_storeTrigKeys" : False,
    "m_derivationName" : "",
    "m_checkDuplicatesData" : True,
    "m_checkDuplicatesMC" : True,
    "m_metaDataStreamName" : "trees",
    "m_cutFlowStreamName" : "trees",
}
